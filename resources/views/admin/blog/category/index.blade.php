...
@endsection
@section('content')
    @include('admin.blog.category.categoryModal')
            ...
            <div class="col-md-12">
                <div class="card ">
                    <div class="card-header p-3">
                        <h4 class=" d-flex justify-content-between"> Blog Category List
                            <a id="add_btn" onclick="addNewBtn('Add Category','Save')" class="btn btn-outline-primary">Add</a>
                        </h4>

                    </div>
                    <div class="card-body">
                        <table class="table table-sm" id="blog-category-datatables">
                            <thead>
                                <tr>
                                    <th>Sl</th>
                                    <th>Name</th>
...
@push('scripts')
    <script>
        ...
        let table = $('#blog-category-datatables').DataTable({
            processing: true,
            serverSide: true,
            ...
            ajax: {
                url: "{{ route('admin.blog.category.get-data') }}",
                type: "POST",
                dataType: "JSON",
                data: function(d) {
                    d._token = _token
        ...
        $(document).on("click",'button#edit-btn',function(e) {
            ...
            $.ajax({
                type: "post",
                url: "{{ route('admin.blog.category.edit') }}",
        ...
        $(document).on('click','button#delete-btn',function(e) {
            e.preventDefault();
            let url = "{{ route('admin.blog.category.destroy') }}";
            let data_id = $(this).data('id');
