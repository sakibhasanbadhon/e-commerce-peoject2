...
@endsection
@section('content')
    @include('admin.pickupPoint.modal')
            ...
            <div class="col-md-12">
                <div class="card ">
                    <div class="card-header p-3">
                        <h4 class=" d-flex justify-content-between"> Pickup Point List
                            <a id="add_btn" onclick="addNewBtn('Add New','Save')" class="btn btn-outline-primary">Add</a>
                        </h4>

                    </div>
                    <div class="card-body">
                        <table class="table table-sm" id="pickup-datatables">
                            <thead>
                                <tr>
                                    <th>Sl</th>
...
@push('scripts')
    <script>
        ...
        let table = $('#pickup-datatables').DataTable({
            processing: true,
            serverSide: true,
            ...
            ajax: {
                url: "{{ route('admin.pickupPoint.get-data') }}",
                type: "POST",
                dataType: "JSON",
                data: function(d) {
                    d._token = _token
        ...
        $(document).on("submit",'form#ajaxForm',function(e) {
            e.preventDefault();
            let url = "{{ route('admin.pickupPoint.store') }}";
        ...
        $(document).on("click",'button#edit-btn',function(e) {
            e.preventDefault();
            let url = "{{ route('admin.pickupPoint.edit') }}";
            let data_id = $(this).data('id');

            $('.modal').modal('show');
            $('#modalTitle').text('Edit Pickup Point');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response) {
                        $('form#ajaxForm input[name="pickup_point_name"]').val(response.pickup_point_name);
                        $('form#ajaxForm input[name="address"]').val(response.address);
                        $('form#ajaxForm input[name="phone"]').val(response.phone);
                        $('form#ajaxForm input[name="another_phone"]').val(response.another_phone);
        ...
        $(document).on('click','button#delete-btn',function(e) {
            e.preventDefault();
            let url = "{{ route('admin.pickupPoint.destroy') }}";
            let data_id = $(this).data('id');
